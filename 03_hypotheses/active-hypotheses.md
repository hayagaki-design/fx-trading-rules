...
| HYP-009-lock-1.5R（派生） | v0.1 | UNKNOWN（未固定） | UNKNOWN（HYPOTHESIS・正式採用不可） |
...
## HYP-009派生 — 1.5Rロックで継続

[HYP-009-lock-1.5R v0.1](HYP-009-lock-1.5R-v0.1.md)はHYPOTHESIS。通常009の固定1.5R出口と矛盾するため、通常条件を上書きせず別識別子・別観察記録で管理する。対象保有の明示宣言とストップ有効化が必要。既決済の付け替えは禁止。元のRと12本期限を保持する。

通常009と派生の統計、実注文・デモ・仮想は分離する。日次実注文制限のみ共有し二重計上しない。採用基準等は未固定、検証0件、正式採用0件。

履歴：2026-09-15 — 派生案と[チャート判定正本](../01_principles/chart-assessment-canonical.md)を追加。観察→仮説→条件固定→検証→採用／却下の順を維持する。既存HYP-008・通常HYP-009の条件・状態は変更なし。
